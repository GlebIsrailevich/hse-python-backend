...
class CartItem:
    ...
    def createCartItem(idGen=_id_cart_item_generator):
        return CartItem(
            id=next(idGen), name="CartItem", quantity=0, available=True, price=0
        )
...
class Item:
    ...
    def createItem(idGen=_id_item_generator):
        return Item(id=next(idGen), name="New Item", price=0, deleted=False)
    ...
    def toCartItem(self):
        return CartItem(
            id=self.id, name=self.name, quantity=1, available=True, price=self.price
        )
